...
async def patient_ledger(
    ...
    ledger = await LedgerService.get_patient_ledger(
        db, ctx.clinic_id, patient_id, currency=ctx.clinic.currency, limit=limit, offset=offset
    )
    return ApiResponse(data=ledger)


@router.get("/patients/{patient_id}/balance", response_model=ApiResponse[PatientBalanceResponse])
async def patient_balance(
    patient_id: UUID,
    ctx: Annotated[ClinicContext, Depends(get_clinic_context)],
    _: Annotated[None, Depends(require_permission("payments.record.read"))],
    db: Annotated[AsyncSession, Depends(get_db)],
) -> ApiResponse[PatientBalanceResponse]:
    """Instant real-time balance for cashier without opening full ledger (concurrency safe)."""
    balance = await LedgerService.get_patient_balance(
...
@router.post(
    "/patients/{patient_id}/quick-charge",
    response_model=ApiResponse[dict],
    status_code=status.HTTP_201_CREATED,
)
async def patient_quick_charge(
    patient_id: UUID,
    payload: QuickChargeCreate,
    ctx: Annotated[ClinicContext, Depends(get_clinic_context)],
    _: Annotated[None, Depends(require_permission("payments.record.create"))],
    db: Annotated[AsyncSession, Depends(get_db)],
) -> ApiResponse[dict]:
    """Add a quick direct clinical charge (e.g. checkup, direct service) directly to the ledger."""
    charge = await LedgerService.create_quick_charge(
        db=db,
        clinic_id=ctx.clinic_id,
        patient_id=patient_id,
        user_id=ctx.user_id,
    ...
    return ApiResponse(
        data={
            ...
            "performed_at": charge.performed_at.isoformat(),
        }
    )
