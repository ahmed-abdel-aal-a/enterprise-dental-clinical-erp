...
@router.post(
    "/first-visit",
    response_model=ApiResponse[FirstVisitResponse],
    status_code=status.HTTP_201_CREATED,
)
async def first_visit_onboarding(
    payload: FirstVisitCreate,
    ctx: Annotated[ClinicContext, Depends(get_clinic_context)],
    _: Annotated[None, Depends(require_permission("patients.write"))],
    db: Annotated[AsyncSession, Depends(get_db)],
) -> ApiResponse[FirstVisitResponse]:
    """Atomic first-visit onboarding: creates patient, initial checkup charge, and payment in a single All-or-Nothing transaction."""
    ...
    patient_dict = payload.patient_data.model_dump(exclude_unset=True)
    if "billing_address" in patient_dict and patient_dict["billing_address"]:
        if hasattr(payload.patient_data.billing_address, "model_dump"):
            patient_dict["billing_address"] = payload.patient_data.billing_address.model_dump()
    ...
    async with transaction_context:
        ...
        # 3. Record initial payment if collected
        if payload.initial_payment:
            branch_id = payload.branch_id
            if not branch_id:
                branch_res = await db.execute(
                    select(ClinicBranch.id)
                    .where(ClinicBranch.clinic_id == ctx.clinic_id)
                    .order_by(ClinicBranch.is_main.desc(), ClinicBranch.created_at.asc())
                    .limit(1)
                )
                branch_id = branch_res.scalar_one_or_none()
