...
def _persist_api_key_to_env(key_name: str, value: str) -> None:
...
class CopilotSettingsService:
    ...
    async def get_or_create(db: AsyncSession, clinic_id: UUID) -> CopilotSettings:
        ...
        default_provider = getattr(app_settings, "COPILOT_PROVIDER_DEFAULT", "gemini")
        if default_provider == "gemini":
            default_model = getattr(app_settings, "COPILOT_MODEL_CHAT_GEMINI", "gemini-3.6-flash")
        elif default_provider == "groq":
            default_model = getattr(app_settings, "COPILOT_MODEL_CHAT_GROQ", "openai/gpt-oss-120b")
        elif default_provider == "ollama":
            default_model = getattr(app_settings, "COPILOT_MODEL_CHAT_OLLAMA", "llama3.2")
        else:
            default_model = getattr(app_settings, "COPILOT_MODEL_CHAT_OPENAI", "gpt-5.4-mini")
    ...
    async def update(
        ...
        target_provider = (data.get("provider") or row.provider).lower()

        # Handle API key updates if supplied
        ...
        if raw_key is not None:
            ...
            if target_provider == "gemini":
                ...
                _persist_api_key_to_env("GEMINI_API_KEY", api_key)
            elif target_provider == "groq":
                app_settings.GROQ_API_KEY = api_key
                os.environ["GROQ_API_KEY"] = api_key
                _persist_api_key_to_env("GROQ_API_KEY", api_key)
            elif target_provider == "openai":
                app_settings.OPENAI_API_KEY = api_key
                os.environ["OPENAI_API_KEY"] = api_key
                _persist_api_key_to_env("OPENAI_API_KEY", api_key)

        # Validate OpenAI key if selecting OpenAI
        if target_provider == "openai" and not (raw_key or app_settings.OPENAI_API_KEY):
            raise ValueError("OpenAI provider selected but OPENAI_API_KEY is not configured")

        # Automatically assign sensible default model if provider changed and model was not explicitly set
        if "provider" in data and ("model" not in data or not data["model"]):
            if target_provider == "gemini":
                data["model"] = getattr(app_settings, "COPILOT_MODEL_CHAT_GEMINI", "gemini-3.6-flash")
            elif target_provider == "groq":
                data["model"] = getattr(app_settings, "COPILOT_MODEL_CHAT_GROQ", "openai/gpt-oss-120b")
            elif target_provider == "ollama":
                data["model"] = getattr(app_settings, "COPILOT_MODEL_CHAT_OLLAMA", "llama3.2")
            elif target_provider == "openai":
                data["model"] = getattr(app_settings, "COPILOT_MODEL_CHAT_OPENAI", "gpt-5.4-mini")
