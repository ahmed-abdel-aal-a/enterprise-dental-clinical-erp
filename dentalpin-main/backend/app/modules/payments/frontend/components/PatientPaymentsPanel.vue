...
import PatientQuickLedger from './PatientQuickLedger.vue'

const activeView = ref<'quick' | 'detailed'>('quick')
...
<template>
  <div class="space-y-4 pb-20 lg:pb-0">
    <!-- View Switcher: Quick Ledger (Default) vs Detailed Accounting -->
    <div class="flex items-center justify-between bg-gray-50 dark:bg-gray-800/60 p-2 rounded-xl border border-gray-200 dark:border-gray-700" dir="rtl">
      <div class="flex items-center gap-2">
        <UIcon name="i-lucide-receipt-text" class="w-4 h-4 text-primary-600" />
        <span class="text-xs font-bold text-gray-700 dark:text-gray-300">نمط عرض الحسابات:</span>
      </div>
      <div class="flex gap-1.5">
        <UButton
          size="xs"
          :color="activeView === 'quick' ? 'primary' : 'gray'"
          :variant="activeView === 'quick' ? 'solid' : 'ghost'"
          icon="i-lucide-zap"
          @click="activeView = 'quick'"
        >
          كشف الحساب والمدفوعات السريع (موصى به)
        </UButton>
        <UButton
          size="xs"
          :color="activeView === 'detailed' ? 'primary' : 'gray'"
          :variant="activeView === 'detailed' ? 'solid' : 'ghost'"
          icon="i-lucide-file-text"
          @click="activeView = 'detailed'"
        >
          نظام الفواتير والموازنات التفصيلي
        </UButton>
      </div>
    </div>

    <!-- 1. Quick Ledger View (Default) -->
    <div v-if="activeView === 'quick'">
      <PatientQuickLedger :patient-id="ctx.patientId" :patient-name="patientFullName" />
    </div>

    <!-- 2. Detailed Accounting View -->
