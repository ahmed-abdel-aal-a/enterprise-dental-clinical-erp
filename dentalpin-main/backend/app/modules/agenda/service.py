...
class CabinetService:
    ...
    async def list_cabinets(
        ...
        result = await db.execute(
            query.order_by(Cabinet.display_order, Cabinet.name)
        )
    ...
    async def create_cabinet(db: AsyncSession, clinic_id: UUID, data: dict) -> Cabinet:
        ...
        if data.get("display_order") is None:
            # Append at the end.
            filter_args = [Cabinet.clinic_id == clinic_id]
            if data.get("branch_id"):
                filter_args.append(Cabinet.branch_id == data["branch_id"])
            max_order = (
                await db.execute(
...
class AppointmentService:
    ...
    async def list_appointments(
        ...
        *,
        ...
        if branch_id:
            count_filters.append(Appointment.branch_id == branch_id)
        if start_date:
    ...
    async def _resolve_cabinet(db: AsyncSession, clinic_id: UUID, data: dict) -> None:
        ...
        if cab is not None:
            if data.get("branch_id") and cab.branch_id and cab.branch_id != data["branch_id"]:
                raise ValueError(f"Cabinet '{cab.name}' belongs to a different branch")
            data["cabinet_id"] = cab.id
            data["cabinet"] = cab.name
            if not data.get("branch_id") and cab.branch_id:
                data["branch_id"] = cab.branch_id
        else:
    ...
    async def assign_cabinet(
        ...
        if cabinet_id is not None:
            ...
            if target_cabinet is None:
                raise ValueError(f"Cabinet not found: {cabinet_id}")
            if appointment.branch_id and target_cabinet.branch_id and target_cabinet.branch_id != appointment.branch_id:
                raise ValueError(f"Cabinet '{target_cabinet.name}' belongs to a different branch")
