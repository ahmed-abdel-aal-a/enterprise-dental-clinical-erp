...
async def setup(
    ...
    timezone = data.timezone or (preset.timezone if data.country else "Africa/Cairo")
    currency = data.currency or (preset.currency if data.country else "EGP")
    ...
    main_branch = ClinicBranch(
        clinic_id=clinic.id,
        name="الفرع الرئيسي" if language == "ar" else "Main Branch",
        code="MAIN",
        is_main=True,
        is_active=True,
        display_order=0,
...
class _CommunicationsSettingsPatch(BaseModel):
    language: str | None = Field(default=None, pattern="^(es|en|fr|pt|ta|ar)$")
