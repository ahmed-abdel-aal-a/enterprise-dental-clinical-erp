...
async def record_payment(
    ...
    *,
    ...
    # Lock referenced Budgets first (in stable sorted order) before Patient
    # to avoid ABBA deadlocks with invoice cobros (which lock Budget -> Invoice -> Patient)
    budget_ids = sorted({
        a["target_id"]
        for a in allocations
        if a.get("target_type") == "budget" and a.get("target_id") is not None
    })
    for bid in budget_ids:
        await db.execute(
            select(Budget.id)
            .where(Budget.id == bid, Budget.clinic_id == clinic_id)
            .with_for_update()
        )

    # Row-Level Lock on Patient to prevent concurrent financial race conditions
