...
<p><strong>${escapeHtml(f.producer_name)}</strong>, con NIF <strong>${escapeHtml(f.producer_nif)}</strong>, en su condición de <strong>productor del Sistema Informático de Facturación</strong> denominado <strong>DentApex</strong> (identificador del sistema <strong>${escapeHtml(f.producer_id_sistema)}</strong>, versión <strong>${escapeHtml(f.producer_version)}</strong>), declara responsablemente que:</p>
...
<div class="meta">
  ...
  Sistema: DentApex · ID ${escapeHtml(f.producer_id_sistema)} · v${escapeHtml(f.producer_version)}
</div>
