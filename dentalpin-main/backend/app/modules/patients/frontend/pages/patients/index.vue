...
// --- Create modal (preserved + First-Visit quick onboarding) -----------
...
// Quick First-Visit Fee & Immediate Collection
const withFirstVisit = ref(false)
const firstVisitAmount = ref(200)
const firstVisitDescription = ref('كشف زيارة أولى واستشارة')
const firstVisitCollectNow = ref(true)
const firstVisitPaymentMethod = ref<'cash' | 'card' | 'bank_transfer'>('cash')
...
function resetForm() {
  ...
  withFirstVisit.value = false
  firstVisitAmount.value = 200
  firstVisitDescription.value = 'كشف زيارة أولى واستشارة'
  firstVisitCollectNow.value = true
  firstVisitPaymentMethod.value = 'cash'
}
...
async function createPatient() {
  ...
  try {
    ...
    if (withFirstVisit.value && Number(firstVisitAmount.value) > 0) {
      // Atomic First-Visit Onboarding
      const payload: Record<string, unknown> = {
        ...
        initial_charge: {
          amount: Number(firstVisitAmount.value),
          description: firstVisitDescription.value || 'كشف زيارة أولى واستشارة'
        }
      }

      if (firstVisitCollectNow.value) {
        payload.initial_payment = {
          amount: Number(firstVisitAmount.value),
          method: firstVisitPaymentMethod.value,
          notes: 'سداد كشف الزيارة الأولى'
        }
      }

      const response = await api.post<ApiResponse<{ patient: Patient }>>('/api/v1/patients/first-visit', payload)
      createdPatientId = response.data.patient.id
      toast.add({
        title: t('common.success'),
        description: 'تم تسجيل المريض وحساب كشف الزيارة الأولى بنجاح',
        color: 'success'
      })
    } else {
    ...
    resetForm()
    await refresh()
    await router.push(`/patients/${createdPatientId}`)
  } catch (e: unknown) {
    ...
    <template #content>
      <UCard>
        ...
        >
          ...
          <!-- First-Visit Checkup & Instant Cashier (Master Blueprint v2) -->
          <div class="border border-primary/30 dark:border-primary/40 bg-primary-50/50 dark:bg-primary-950/20 rounded-xl p-3.5 space-y-3">
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-2.5">
                <div class="w-8 h-8 rounded-lg bg-primary-100 dark:bg-primary-900/60 flex items-center justify-center text-primary-600 dark:text-primary-400">
                  <UIcon name="i-lucide-stethoscope" class="w-4 h-4" />
                </div>
                <div>
                  <div class="font-medium text-sm text-default">تسجيل كشف الزيارة الأولى فوري</div>
                  <div class="text-xs text-subtle">إنشاء قيد الكشف وتسهيل التحصيل الفوري مع فتح الملف مباشرة</div>
                </div>
              </div>
              <USwitch v-model="withFirstVisit" />
            </div>

            <div v-if="withFirstVisit" class="pt-3 border-t border-primary/20 dark:border-primary/30 space-y-3">
              <div>
                <div class="flex items-center justify-between mb-1.5">
                  <label class="text-xs font-semibold text-default">قيمة الكشف / الاستشارة</label>
                  <div class="flex gap-1.5">
                    <button
                      v-for="amt in [150, 200, 250, 300]"
                      :key="amt"
                      type="button"
                      class="px-2.5 py-0.5 text-xs rounded-md border transition-all"
                      :class="firstVisitAmount === amt ? 'bg-primary-600 text-white border-primary-600 font-semibold shadow-xs' : 'bg-surface border-border text-subtle hover:bg-surface-muted'"
                      @click="firstVisitAmount = amt"
                    >
                      {{ amt }} ج.م
                    </button>
                  </div>
                ...
                <UInput
                  v-model.number="firstVisitAmount"
                  type="number"
                  placeholder="200"
                  icon="i-lucide-coins"
                />
              ...
              <div class="flex items-center justify-between pt-1">
                <label class="text-xs font-medium text-default flex items-center gap-1.5">
                  <UIcon name="i-lucide-receipt" class="w-4 h-4 text-success" />
                  تحصيل المبلغ نقداً / فوري الآن في الخزينة
                </label>
                <USwitch v-model="firstVisitCollectNow" />
              </div>

              <div v-if="firstVisitCollectNow" class="grid grid-cols-3 gap-2 pt-1">
                <button
                  type="button"
                  class="py-2 px-2 text-xs rounded-lg border flex items-center justify-center gap-1.5 transition-all"
                  :class="firstVisitPaymentMethod === 'cash' ? 'bg-emerald-50 dark:bg-emerald-950/50 border-emerald-500 text-emerald-700 dark:text-emerald-300 font-bold shadow-xs' : 'bg-surface border-border text-subtle hover:bg-surface-muted'"
                  @click="firstVisitPaymentMethod = 'cash'"
                >
                ...
                <button
                  type="button"
                  class="py-2 px-2 text-xs rounded-lg border flex items-center justify-center gap-1.5 transition-all"
                  :class="firstVisitPaymentMethod === 'card' ? 'bg-emerald-50 dark:bg-emerald-950/50 border-emerald-500 text-emerald-700 dark:text-emerald-300 font-bold shadow-xs' : 'bg-surface border-border text-subtle hover:bg-surface-muted'"
                  @click="firstVisitPaymentMethod = 'card'"
                >
                ...
                <button
                  type="button"
                  class="py-2 px-2 text-xs rounded-lg border flex items-center justify-center gap-1.5 transition-all"
                  :class="firstVisitPaymentMethod === 'bank_transfer' ? 'bg-emerald-50 dark:bg-emerald-950/50 border-emerald-500 text-emerald-700 dark:text-emerald-300 font-bold shadow-xs' : 'bg-surface border-border text-subtle hover:bg-surface-muted'"
                  @click="firstVisitPaymentMethod = 'bank_transfer'"
                >
