...
export function usePayments() {
  ...
  async function fetchPatientBalance(patientId: string): Promise<PatientBalance | null> {
    try {
      const resp = await api.get<ApiResponse<PatientBalance>>(`/api/v1/payments/patients/${patientId}/balance`)
      return resp.data
    } catch {
      return null
    }
  ...
  async function createQuickCharge(
    patientId: string,
    payload: { amount: number; description: string; catalog_item_id?: string; notes?: string }
  ): Promise<{ success: boolean; data?: any; error?: string }> {
    try {
      const resp = await api.post<ApiResponse<any>>(
        `/api/v1/payments/patients/${patientId}/quick-charge`,
        payload
      )
  ...
  async function recordQuickPayment(
    ...
    try {
      const resp = await api.post<ApiResponse<any>>('/api/v1/payments/', {
