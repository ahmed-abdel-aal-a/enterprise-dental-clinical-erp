...
class LedgerService:
    """Patient-level KPIs + chronological timeline with hardened SQL Window Running Balance."""
    ...
    async def get_patient_ledger(
        ...
        timeline = await LedgerService._build_timeline(
            db, clinic_id, patient_id, limit=limit, offset=offset
        )
    ...
    async def get_patient_balance(
        ...
        """Instant real-time balance for cashier without opening full ledger."""
        total_paid_row = await db.execute(
            select(
                func.coalesce(func.sum(Payment.amount), Decimal("0")),
            ).where(Payment.clinic_id == clinic_id, Payment.patient_id == patient_id)
        ...
        net_balance = total_earned - net_paid

        return PatientBalanceResponse(
            patient_id=patient_id,
            currency=currency,
            total_charged=total_earned,
            total_paid=net_paid,
            net_balance=net_balance,
            has_debt=net_balance > Decimal("0"),
            has_credit=net_balance < Decimal("0"),
        )

    @staticmethod
    async def create_quick_charge(
        db: AsyncSession,
        clinic_id: UUID,
        patient_id: UUID,
        user_id: UUID,
        payload: QuickChargeCreate,
    ) -> PatientEarnedEntry:
        """Create an immediate direct clinical charge without requiring an odontogram session."""
    ...
    async def _build_timeline(
        db: AsyncSession, clinic_id: UUID, patient_id: UUID, limit: int = 100, offset: int = 0
    ) -> list[LedgerEntry]:
        """Fetch timeline rows with mathematical SQL Window Function running balance."""
        query = text(
            """
            WITH patient_timeline_events AS (
                -- 1. المكتسبات والرسوم (المدين)
                SELECT 
                    pe.id AS reference_id,
                    'earned' AS entry_type,
                    pe.performed_at AS occurred_at,
                    pe.amount AS charge_amount,
                    0.00 AS paid_amount,
                    pe.amount AS net_impact,
                    COALESCE(pe.description, ci.names->>'ar', ci.names->>'es', ci.names->>'en', pe.source_event) AS description,
                    pe.treatment_id,
                    t.status AS treatment_status,
                    pe.professional_id,
                    TRIM(COALESCE(u.first_name, '') || ' ' || COALESCE(u.last_name, '')) AS professional_name,
                    COALESCE(ci.names->>'ar', ci.names->>'es', ci.names->>'en') AS treatment_name
                ...
                -- 2. المدفوعات والتحصيلات (الدائن)
                SELECT 
                    p.id AS reference_id,
                    'payment' AS entry_type,
                    COALESCE(p.created_at, p.payment_date::timestamptz) AS occurred_at,
                    0.00 AS charge_amount,
                    p.amount AS paid_amount,
                    -p.amount AS net_impact,
                    COALESCE(p.notes, p.method) AS description,
                    NULL::uuid AS treatment_id,
                    NULL AS treatment_status,
                    p.recorded_by AS professional_id,
                    TRIM(COALESCE(u.first_name, '') || ' ' || COALESCE(u.last_name, '')) AS professional_name,
                    NULL AS treatment_name
                FROM payments p
                LEFT JOIN users u ON u.id = p.recorded_by
                WHERE p.clinic_id = :clinic_id AND p.patient_id = :patient_id

                UNION ALL

                -- 3. المبالغ المستردة (إعادة مدين)
                SELECT 
                    ...
                    r.amount AS net_impact,
                    COALESCE(r.reason_note, r.reason_code) AS description,
                    NULL::uuid AS treatment_id,
                    NULL AS treatment_status,
                    r.refunded_by AS professional_id,
                    TRIM(COALESCE(u.first_name, '') || ' ' || COALESCE(u.last_name, '')) AS professional_name,
                    NULL AS treatment_name
                FROM refunds r
                JOIN payments p ON p.id = r.payment_id
                LEFT JOIN users u ON u.id = r.refunded_by
                WHERE r.clinic_id = :clinic_id AND p.patient_id = :patient_id
            ),
            timeline_with_running_balance AS (
                SELECT 
                    *,
                    SUM(net_impact) OVER (
            ...
            SELECT * FROM timeline_with_running_balance
        ...
        for row in result.mappings():
            ...
            charge_amt = Decimal(str(row["charge_amount"] or 0))
            ...
            if entry_type == "earned":
                ...
                amount = paid_amt  # negative

            prof_name = row["professional_name"]
            if prof_name is not None:
                prof_name = prof_name.strip() or None
