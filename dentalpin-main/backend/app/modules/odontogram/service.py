...
class TreatmentService:
    ...
    @staticmethod
    async def _resolve_catalog_item_for_type(
        db: AsyncSession, clinic_id: UUID, clinical_type: str | None
    ) -> TreatmentCatalogItem | None:
        """Find the default or primary catalog item mapped to this odontogram clinical type."""
        if not clinical_type:
            return None
        stmt = (
            select(TreatmentCatalogItem)
            .join(
                TreatmentOdontogramMapping,
                TreatmentOdontogramMapping.catalog_item_id == TreatmentCatalogItem.id,
            )
            .options(
                selectinload(TreatmentCatalogItem.odontogram_mapping),
                selectinload(TreatmentCatalogItem.vat_type_rel),
            ...
            .where(
                TreatmentCatalogItem.clinic_id == clinic_id,
                TreatmentOdontogramMapping.clinic_id == clinic_id,
                TreatmentOdontogramMapping.odontogram_treatment_type == clinical_type,
                TreatmentCatalogItem.deleted_at.is_(None),
            )
            .order_by(
                TreatmentCatalogItem.is_default_for_type.desc(),
                TreatmentCatalogItem.created_at.asc(),
            )
        )
    ...
    async def create(
        ...
        elif clinical_type is not None:
            catalog_item = await TreatmentService._resolve_catalog_item_for_type(
                db, clinic_id, clinical_type
            )
