        ...
        <body>
            ...
            <div class="header">
                <div class="clinic-info">
                    <div class="clinic-name">{clinic.name if clinic else "Dental Clinic"}{f" - {invoice.branch.name}" if getattr(invoice, "branch", None) and getattr(invoice.branch, "name", None) and not getattr(invoice.branch, "is_main", False) else ""}</div>
                    <div class="clinic-details">
                        {
            InvoicePDFService._format_address(invoice.branch.address if getattr(invoice, "branch", None) and invoice.branch.address else getattr(clinic, "address", None))
        }<br>
                        {(invoice.branch.phone if getattr(invoice, "branch", None) and invoice.branch.phone else getattr(clinic, "phone", "")) or ""}
                        {f" | {getattr(clinic, 'email', '')}" if getattr(clinic, "email", None) else ""}
                        {
            ...
            <div class="footer">
                {labels["generated_by"]} DentApex | {generated_at}
            </div>
        </body>
        </html>
        """
