...
async def _load_clinic_weekly(
    ...
    if branch_id:
        result = await db.execute(base_query.where(ClinicWeeklySchedule.branch_id == branch_id))
        schedule = result.scalar_one_or_none()
        if schedule:
            return schedule

    result = await db.execute(base_query.where(ClinicWeeklySchedule.branch_id.is_(None)))
    return result.scalar_one_or_none() or (await db.execute(base_query)).scalars().first()


async def _load_clinic_overrides(
    db: AsyncSession, clinic_id: UUID, start: date, end: date, branch_id: UUID | None = None
) -> list[ClinicOverride]:
    query = (
    ...
    if branch_id:
        query = query.where(
            or_(ClinicOverride.branch_id == branch_id, ClinicOverride.branch_id.is_(None))
        )
    ...
    *,
    ...
    if clinic_override is not None:
        ...
        clinic_shift_intervals = _sorted_shifts(
            [
                ...
                for s in clinic_override.shifts
                if (branch_id is None or s.branch_id is None or s.branch_id == branch_id)
            ]
        )
        ...
        clinic_shift_intervals = _sorted_shifts(
            [
                (s.start_time, s.end_time)
                for s in clinic_weekly.shifts
                if s.weekday == weekday
                and (branch_id is None or s.branch_id is None or s.branch_id == branch_id)
            ]
        )
    ...
    if professional_override is not None:
        ...
        professional_intervals = _sorted_shifts(
            [
                (s.start_time, s.end_time)
                for s in professional_override.shifts
                if (branch_id is None or s.branch_id is None or s.branch_id == branch_id)
            ]
        )
        ...
        professional_intervals = _sorted_shifts(
            [
                (s.start_time, s.end_time)
                for s in professional_weekly.shifts
                if s.weekday == weekday
                and (branch_id is None or s.branch_id is None or s.branch_id == branch_id)
            ]
        )
